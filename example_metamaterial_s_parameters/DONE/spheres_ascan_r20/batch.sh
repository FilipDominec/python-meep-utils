...
thz=1e12
staticpar=(model=SphereWire resolution=4u radius=30e-6 wirethick=0)
par=" cellsize=$cellsize"

#if [ -z "$skipsimulation" ]; then 
    #for cs in `seq 52 8 80` `seq 48 8 120`; do
        #np ../../scatter.py "${staticpar[@]}" cellsize=${cs}e-6
        #../../effparam.py --numstabcoef .97
    #done
#fi
#
plotoptions=(effparam/*.dat  --xlabel     'Frequency (THz)'       --xeval x/1e12 \
        --paramname cellsize  --paramlabel 'Unit cell size $a$' --parameval param*1e6 \
