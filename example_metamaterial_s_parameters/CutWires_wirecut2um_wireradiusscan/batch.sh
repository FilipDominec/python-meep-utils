...
../../plot_multiline.py $sharedoptions --paramlabel '$r_w$ = %.1f $\upmu$m' --xlabel "Frequency (THz)" --ycol 'real N' \
    ...
    --overlayplot "c/2/$cellsize/x/$thz,2*c/2/$cellsize/x/$thz,3*c/2/$cellsize/x/$thz,4*c/2/$cellsize/x/$thz" 
../../plot_multiline.py $sharedoptions --paramlabel '$r_w$ = %.1f $\upmu$m' --xlabel "Frequency (THz)" --yeval '0-y' --ycol 'imag N' \
   	--ylabel 'Refractive index $N_{\text{eff}^{\prime\prime}}$' --output ${PWD##*/}_ni.pdf 


../../plot_multiline.py $sharedoptions --paramlabel '$r_w$ = %.1f $\upmu$m' --xlabel "Frequency (THz)" --ycol 'real eps' --y2eval '0-y2' --ycol2 'imag eps' --ylim1 -5 --ylim2 5  \
...
../../plot_multiline.py effparam/*thick=1.000e-06*.dat --paramname wirethick --parameval param*1e6 --figsizey 2 --xeval x/1e12 \
	--paramlabel '$r_w$ = %.1f $\upmu$m' --xlabel "Frequency (THz)" --ycol 'real eps' --y2eval '0-y2' --ycol2 'imag eps' --ylim1 -5 --ylim2 5  \
   	--ylabel 'Effective permittivity $\varepsilon_{\text{eff}}$' --output ${PWD##*/}_eps_wt1.pdf
../../plot_multiline.py $sharedoptions --paramlabel '$r_w$ = %.1f $\upmu$m' --xlabel "Frequency (THz)" --ycol 'real mu' --y2eval '0-y2' --ycol2 'imag mu' --ylim1 -12 --ylim2 4  \
