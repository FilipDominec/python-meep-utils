...
Nfreq = 20
my_ldos = meep.dft_ldos(model.src_freq-model.src_width, model.src_freq+model.src_width, Nfreq) ## XXX LDOS
...
if not getattr(model, 'frequency', None):       ## time-domain computation
    ...
    while (f.time()/c < model.simtime):     # timestepping cycle
        ...
        my_ldos.update(f) ## XXX LDOS
    for slice_ in slices: slice_.finalize()
    my_ldos_raw = my_ldos.ldos()
    ldos_results = [meep.doubleArray_getitem(my_ldos_raw,i) for i in range(Nfreq)]
    print ldos_results
