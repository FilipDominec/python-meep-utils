...
freq, s11amp, s11phase, s12amp, s12phase, xlim, (d, plot_freq_min, plot_freq_max, padding) = load_rt(last_simulation_name,  
        plot_freq_min=plot_freq_min, plot_freq_max=plot_freq_max, truncate=False, padding=padding)
...
xnumbers = [("%.2f"%(f/frequnit) if abs(f%reasonable_ticks((xlim[1]-xlim[0])/10))<(frequnit/1000) else "") for f in xticks]
...
if plot_publi:
    ...
    if toplot['rt']:
        ...
        subplot_index += 1
        plt.xticks(xticks, xnumbers); plt.minorticks_on(); 
        plt.xlim(xlim); plt.ylim((0,1.)); plt.legend(loc='lower left');
    ...
    if toplot['N']:
        ...
        plt.xticks(xticks, xnumbers); plt.minorticks_on()
        plt.xlim(xlim); plt.ylim((-5,5)); plt.legend(loc='lower left'); 
        subplot_index += 1
